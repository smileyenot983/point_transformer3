...
# {'type': 'Collect', 'keys': ('coord', 'grid_coord', 'index'), 'feat_keys': ('color', 'normal')}
...
class Collect(object):
    def __init__(self, keys, offset_keys_dict=None, **kwargs):
        ...
        self.kwargs = kwargs
        # print(f"[Collect] self.kwargs: {self.kwargs}")
...
class GridSample(object):
    ...
    def __call__(self, data_dict):
        ...
        # every point in 3d has key
        key = self.hash(grid_coord)
        # print(f"len(key): {len(key)}")
        # print(f"np.unique(key).shape: {np.unique(key).shape}")
        # 
        idx_sort = np.argsort(key)
        key_sort = key[idx_sort]
        _, inverse, count = np.unique(key_sort, return_inverse=True, return_counts=True)
        # print(f"count.max(): {count.max()}")
        if self.mode == "train":  # train mode
            ...
            for i in range(count.max()):
                ...
                # print(f"len(idx_select): {len(idx_select)}")
