...
class TesterBase:
    ...
    def build_model(self):
        ...
        print(f"self.cfg.model: {self.cfg.model}")    
    ...
    def build_test_loader(self):
        ...
        # print(f"self.cfg.data.test: {self.cfg.data.test}")
        # print(f"comm.get_world_size(): {comm.get_world_size()}")

        # print(f"self.cfg.batch_size_test_per_gpu: {self.cfg.batch_size_test_per_gpu}")
...
class SemSegTester(TesterBase):
    def test(self):
        ...
        # fragment inference, fragment contains samplings from the cloud
        for idx, data_dict in enumerate(self.test_loader):
            ...
            # print(f"len(fragment_list): {len(fragment_list)}")
            ...
            pred_save_path = "123"
            # if predictions are already made and saved
            if os.path.isfile(pred_save_path):
                ...
                for i in range(len(fragment_list)): #fragment_list size = n_augs * n_voxels
                    ...
                    # print(f"len(fragment_list): {len(fragment_list)}")
                    # print(f"fragment_list[i].keys(): {fragment_list[i].keys()}")
                    ...
                    # print(f"input_dict.keys(): {input_dict.keys()}")
                    # print(f"input_dict['offset']: {input_dict['offset']}")

                    # print(f"input_dict['coord'].shape: {input_dict['coord'].shape}")
                    # print(f"input_dict['grid_coord'].shape: {input_dict['grid_coord'].shape}")
                    # print(f"input_dict['index'].shape: {input_dict['index'].shape}")
                    # print(f"input_dict['offset'].shape: {input_dict['offset'].shape}")
                    # print(f"input_dict['feat'].shape: {input_dict['feat'].shape}")
                ...
                pred_maxed = pred.max(1)[1].data.cpu().numpy()
                # print(f"pred_maxed.shape: {pred_maxed.shape}")
        ...
        # print(f"self.test_loader: {self.test_loader}")
        ...
        if comm.is_main_process():
            ...
            # allAcc = intersection / (target + 1e-10)
            ...
            # print(f"self.cfg.data.num_classes: {self.cfg.data.num_classes}")
